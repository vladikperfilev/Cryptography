...
bool encryptAll()
{
	...
	//Распечатываем сгенерированные подключи ключа 1
	printf("\nБлоки подключей шифрования 1: ");
	...
	//Распечатываем сгенерированные подключи ключа 2
	printf("\nБлока подключей шифрования 2: ");
	for (j=0; j<IDEAROUNDS+1; j++) {
		printf("\nРаунд %d:   ", j+1);
		if (j < IDEAROUNDS)
			for(i=0; i<6; i++)
	...
	//Распечатываем сгенерированные подключи обратного ключа 1
	printf("\nБлоки подключей расшифрования 2: ");
	for (j=0; j<IDEAROUNDS+1; j++) {
		printf("\nРаунд %d:   ", j+1);
		if (j < IDEAROUNDS)
			for(i=0; i<6; i++)
	...
	//Распечатываем сгенерированные подключи обратного ключа 2
	printf("\nБлоки подключей расшифрования 2: ");
	for (j=0; j<IDEAROUNDS+1; j++) {
		printf("\nРаунд %d:   ", j+1);
		if (j < IDEAROUNDS)
	...
	while(fgets(result_sting,sizeof(result_sting),file))
	{
		...
		printf("\n Расшифровка %d байт (%ld блоков)...", BLOCKS*16, BLOCKS);
		fflush(stdout);
		...
		i = l/1000;
		j = l%1000;
		//Если вычисления заняли достаточное количество времени, выводим его
		if(end!=0)
			printf("%d.%03d секунд= %ld байт в секунду\n", i, j, l);

		//Распечатываем проверку шифрации
		printf("\nX %3u  %3u  %3u  %3u  %3u  %3u  %3u %3u",
			bArray[0], bArray[1],  bArray[2], bArray[3], bArray[4], bArray[5],  bArray[6], bArray[7]);
...
bool decryptAll()
{
	...
	/*while (leng<=lSize){*/
		...
		//Осуществляем дешифрацию
			...
			printf("\n Дешифрация %d байт (%ld блоков)...", BLOCKS*16, BLOCKS);
			fflush(stdout);
			//Запоминаем текущий момент времени
			start = clock();
			//Копируем в массив YY значения из bArray
